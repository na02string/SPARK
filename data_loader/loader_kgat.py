...
class DataLoaderKGAT(DataLoaderBase):
    ...
    def __init__(self, args, logging):
        ...
        # 사용자-리뷰 데이터 프레임 (self.df_reviews)를 로딩하도록 한 줄 추가
        self.df_reviews = pd.read_json(
            os.path.join(args.data_dir, args.review_file),  # ex: Reviews_Movies.jsonl
            lines=True, compression=None
        )[["reviewerID","reviewText"]].rename(
            columns={"reviewerID":"user_id","reviewText":"text"}
        )
